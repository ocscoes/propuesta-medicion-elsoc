...
elsoc_4$confianza_gen <- recode(elsoc_4$confianza_gen, "c(-666, -777, -888, -999) = NA")
elsoc_4$altruismo_gen <- recode(elsoc_4$altruismo_gen, "c(-666, -777, -888, -999) = NA")
elsoc_4$reunion_pub <- recode(elsoc_4$reunion_pub, "c(-666, -777, -888, -999) = NA")
elsoc_4$voluntariado <- recode(elsoc_4$voluntariado, "c(-666, -777, -888, -999) = NA")
elsoc_4$donar_dinero <- recode(elsoc_4$donar_dinero, "c(-666, -777, -888, -999) = NA")
elsoc_4$prestar_dinero <- recode(elsoc_4$prestar_dinero, "c(-666, -777, -888, -999) = NA")
elsoc_4$ayuda_trabajo <- recode(elsoc_4$ayuda_trabajo, "c(-666, -777, -888, -999) = NA")
elsoc_4$frecuencia_migrantes <- recode(elsoc_4$frecuencia_migrantes, "c(-666, -777, -888, -999) = NA")
elsoc_4$contacto_migrantes <- recode(elsoc_4$contacto_migrantes, "c(-666, -777, -888, -999) = NA")
...
saveRDS(elsoc_4, file = "input/data/proc_data/elsoc_2019_coh_mig.RData")