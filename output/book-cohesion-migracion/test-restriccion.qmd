...
# Parámetro para ajustar la altura de los porcentajes
# Puedes cambiar estos valores para cada barra individualmente
altura_porcentajes <- c(0.21, 0.26, 0.28)  # Para barras 1, 2 y 3 respectivamente
...
elsoc_6_labels <- elsoc_6_graph %>%
  ...
  mutate(
    ...
    # Agregar las alturas personalizadas para cada barra
    altura_custom = altura_porcentajes[restriccion_migrantes]
  )
...
ggplot(elsoc_6_graph, aes(
  ...
  geom_text(
    data = elsoc_6_labels,
    aes(x = restriccion_migrantes, y = altura_custom, label = paste0(percentage, "%")),
    inherit.aes = FALSE,
    color = "#2b2f3c",
    fontface = "bold",
    size = 4
  ...
  labs(
    ...
    caption = "Fuente: elaboración propia con datos ELSOC 2022"
  ) +
  theme_minimal() +
  scale_y_continuous(
  ...
  theme_ggdist() +
  theme(
    # Eliminar completamente la leyenda
...
# Parámetro para ajustar la altura de los porcentajes
# Puedes cambiar estos valores para cada barra individualmente
altura_porcentajes <- c(0.07, 0.17, 0.16)  # Para barras 1, 2 y 3 respectivamente
...
# Parámetro para ajustar la altura de los porcentajes
# Puedes cambiar estos valores para cada barra individualmente
altura_porcentajes <- c(0.73, 0.80, 0.85)  # Para barras 1, 2 y 3 respectivamente
