...
```{r}
pacman::p_load(performance, lme4, texreg)

db <- db %>% 
  mutate(ola_2=as.numeric(ola)^2)
...
m0 <- lmer(seguridad_sub ~ 1 + (1 | idencuesta), 
                data = df_study1, weights = ponderador_long_total)

performance::icc(m0, by_group = T)

m1 <- lmer(seguridad_sub ~ 1 + wave + (1 | idencuesta),
                data = df_study1, weights = ponderador_long_total)

screenreg(m2)

m1.1 <- lmer(seguridad_sub ~ 1 + ola + ola_2 + (1 | idencuesta),
                data = df_study1, weights = ponderador_long_total)

m2 <- lmer(seguridad_sub ~ 1 + ola + ola_2 + (1 + ola | idencuesta),
                data = df_study1, weights = ponderador_long_total)

anova(m1.1, m2)

m3 <- lmer(seguridad_sub ~ 1 + ola + ola_2 + perdida_identidad + (1 + ola | idencuesta), 
           data = df_study1, weights = ponderador_long_total)

m4 <- lmer(seguridad_sub ~ 1 + ola + ola_2 + perdida_identidad*ola + (1 + ola | idencuesta), 
           data = df_study1, weights = ponderador_long_total)
