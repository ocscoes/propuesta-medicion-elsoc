...
pacman::p_load(
  ...
  ggtext, patchwork, psych, kableExtra, labelled, ggdist)
```
...
```{r}
#| label: fig-migracion-olaseis
#| fig-cap: Distribución de los ítems de migración ola 2022
...
desc_migracion <- plot_stackfrq(
  ...
  labs(caption = "Elaboración Propia a partir de ELSOC 2022")

desc_migracion
```

En la @fig-migracion-olaseis se muestra la distribución de respuestas de las variables de migración disponibles en la última ola, las cuales han sido seleccionadas para este estudio. Todas las variables son medidas a partir de una escala likert de 1 a 5 (con etiquetas distintas), pero son agregadas en tres categorías para que pueda observarse de mejor forma su distribución. 

En el caso de contacto con inmigrantes se mide cuán amistosa ha sido la experiencia de relacionarse con alguien extranjero. La mayoría de las personas considera que su interacción con algún inmigrante ha sido positiva. 

Igualdad en acceso a salud mide qué tan de acuerdo se está con que los inmigrantes y chilenos tengan un acceso igualitario al sistema de salud. Más del 80% de las personas cree que la salud debe ser un servicio que no discrimine por nacionalidad. 

Simpatía hacia inmigrantes busca cuantificar la simpatía que tienen las personas hacia los inmigrantes. En este caso, la distribución de las respuestas es bastante alta y equitativa entre las categorías “Muy en desacuerdo” y “Ni en desacuerdo ni de acuerdo”, mientras que las personas que consideran muy simpáticos a los inmigrantes son una minoría.

Las dos últimas variables miden el grado de acuerdo respecto a las sentencias Con la llegada de inmigrantes aumenta el desempleo y Chile pierde su identidad con la llegada de inmigrantes. Estos son los indicadores más llamativos, puesto que aproximadamente el 40% de los encuestados percibe que los inmigrantes erosionan el país en sus dimensiones cultural y laboral.

```{r}
#| label: fig-mig1
#| fig-cap: "Tendencia Variables sobre Migración (2016-2023)"
#| fig-cap-location: top
#| warning: false
#| message: false
#| echo: false
...
ggplot(long_promedio, aes(x = ola, y = promedio, color = variable, group = variable, shape = variable)) +
  ...
  labs(x = "Ola", 
       ...
       color = "Variable", 
       shape = "Variable",
       caption = "Elaboración Propia a partir de ELSOC 2019-2023") +
  theme_minimal() +
  theme(
...
```{r}
#| label: fig-mig2
#| fig-cap: "Distribución variables sobre Migración (2016-2023)"
#| fig-cap-location: top
#| warning: false
#| message: false
#| echo: false
...
```{r}
#| label: fig-mig4
#| fig-cap: "Distribución Respuestas sobre Acceso igualitario de migrantes al sistema de salud (2016-2023)"
#| fig-cap-location: top
#| warning: false
#| message: false
#| echo: false
