...
elsoc_1 <- readRDS(
  here("input", "data", "proc_data", "elsoc_2016_coh_mig.RData")
)

elsoc_2 <- readRDS(
  here("input", "data", "proc_data", "elsoc_2017_coh_mig.RData")
)

elsoc_3 <- readRDS(
  here("input", "data", "proc_data", "elsoc_2018_coh_mig.RData")
)

elsoc_4 <- readRDS(
  here("input", "data", "proc_data", "elsoc_2019_coh_mig.RData")
)

elsoc_5 <- readRDS(
  here("input", "data", "proc_data", "elsoc_2021_mig.RData")
)

elsoc_6 <- readRDS(
  here("input", "data", "proc_data", "elsoc_2022_mig.RData")
)
...
db_long <- here("input/data/proc_data/db_long.RData")
load(db_long)
```
...
db_pond %>% 
  select(wave, satisfaccion_barrio, restriccion_migrantes) %>%
  mutate(satisfaccion_barrio = case_when(
    satisfaccion_barrio <= 2.5 ~ "Baja",
    satisfaccion_barrio >= 3   ~ "Alta"
  ),
  ...
  ggplot(aes(x = restriccion_migrantes, y = prop, fill = satisfaccion_barrio)) +
  ...
  scale_y_continuous(
    ...
    labs(
      title = "Satisfacción con el barrio y restricción de acceso a migrantes",
...
```{r}
#| label: fig-sentper
#| fig-cap: Sentido pertenencia y pérdida de identidad del país debido a inmigrantes
#| fig-cap-location: top
#| echo: false 
#| fig.width: 12
#| fig.height: 8
...
db_pond %>% 
  ...
  scale_y_continuous(
    ...
    labs(
      title = "Sentido de pertenencia y pérdida de identidad debido a migrantes",
...
datos_porcentajes <- datos_combinados %>%
  group_by(grupo, perdida_identidad, sentido_pertenencia) %>%
  summarise(n = n(), .groups = "drop") %>%
  group_by(grupo, perdida_identidad) %>%
  mutate(
    total = sum(n),
    porcentaje = (n / total) * 100,
    pertenencia_label = factor(sentido_pertenencia, labels = c("Alta pertenencia", "Baja pertenencia"))
  )
...
ggplot(datos_porcentajes, aes(
  ...
  labs(
    title = "Percepción de pérdida de identidad y \n sentido de pertenencia por género ",
...
En @fig-terra-gen se grafica la asociación, diferenciada por género, entre sentido de pertenencia y la percepción con la pérdida de identidad nacional a causa de migrantes. Este gráfico sigue la misma lógica que los anteriores, pero su aporte reside en que, al separar por grupos, se pueden analizar dinámicas y patrones propios de cada uno de estos.
...
# Primero calcular los porcentajes para mostrar en las barras
datos_porcentajes <- datos_combinados %>%
  group_by(grupo, desempleo_migrantes, sentido_pertenencia) %>%
  summarise(n = n(), .groups = "drop") %>%
...
ggplot(datos_porcentajes, aes(
  ...
  labs(
    title = "Percepción de desempleo debido a inmigrantes y \n sentido de pertenencia por nivel educativo",
...
# Selección de variables: (1) desempleo por migrantes, (2)pérdida de identidad
# (3)simpatía hacia migrantes, (4)acceso a salud igualitario, (5) contacto positivo y (6) restricción de ingreso a migrantes
...
#| fig-cap: "Satisfacción con el barrio y simpatía hacia migrantes (2016-2023)"
#| fig-cap-location: top

walk(sel[c(11), ]$plot, print)
```
...
#| out-width: '100%'
#| results: asis
#| label: fig-satis-restr
#| fig-cap: "Satisfacción con el barrio y restricción a ingreso de migrantes (2016-2023)"
#| fig-cap-location: top
