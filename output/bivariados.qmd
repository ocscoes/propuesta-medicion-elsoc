---
title: "Cohesión horizontal y migración: descriptivos y correlaciones"
...
pacman::p_load(
  ...
  ggtext, patchwork, psych, kableExtra, labelled)
```
...
graph_in1 <- plot_stackfrq(
  dplyr::select(elsoc_6, desempleo_migrantes),
  title = "Acuerdo en desempleo debido a inmigrantes"
) +
...
graph_in2 <- plot_stackfrq(
  dplyr::select(elsoc_6, perdida_identidad),
...
elsoc_6 <- elsoc_6 %>%
  mutate(seguridad_obj = rowMeans(select(., peleas_calle, asaltos, trafico_drogas), na.rm = TRUE),
    seguridad_obj = round(seguridad_obj * 2) / 2
  )
```
...
elsoc_6 <- elsoc_6 %>%
  mutate(
    ...
    sentido_pertenencia = round(sentido_pertenencia * 2) / 2
  )
```
...
elsoc_6 <- elsoc_6 %>%
  mutate(satisfaccion_barrio = rowMeans(select(., barrio_amigos, barrio_sociable, barrio_cordial, barrio_colaborador), na.rm = TRUE),
    satisfaccion_barrio = round(satisfaccion_barrio * 2) / 2
  )
```
