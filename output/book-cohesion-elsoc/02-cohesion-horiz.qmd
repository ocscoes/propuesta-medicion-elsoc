...
### Esquema de Cohesión Vertical ELSOC

```{r}
#| label: fig-elsoc
#| fig-cap-location: top
#| fig-cap: "Medición de cohesión horizontal con ELSOC de Castillo et al.(2021)"
#| fig-align: center

library(here)

knitr::include_graphics(path = here("output/book-cohesion-elsoc/images/hor-elsoc.png"))
...
plot_stackfrq(
  dplyr::select(db_filtrado, barrio_ideal, barrio_integracion, barrio_identidad, barrio_pertenencia,
...
territorial <- db_filtrado[, c("barrio_ideal", "barrio_integracion", "barrio_identidad", "barrio_pertenencia",
              "barrio_amigos", "barrio_sociable", "barrio_cordial", "barrio_colaborador")]
...
elsoc_redes <- db %>%
  filter(ola == 5) %>% # A diferencia de las anteriores, la ola 5 es la última en la que se encuentran todas las variables de redes sociales
...
## Análisis Factoria Confirmatorio
...
indices <- fitmeasures(seguridad_cfa, c("rmsea", "cfi", "tli"))
...
cat("RESUMEN DEL MODELO CFA")
indices <- fitmeasures(vinculos_cfa, c("rmsea", "cfi", "tli"))
cat("RMSEA =", round(indices["rmsea"], 3), "\n")
cat("CFI =", round(indices["cfi"], 3), "\n")
cat("TLI =", round(indices["tli"], 3), "\n\n")

cat("Cargas factoriales estandarizadas:\n")
standardizedSolution(vinculos_cfa) %>% 
  dplyr::filter(op == "=~") %>% 
  dplyr::select(lhs, rhs, est.std) %>%
  print()
```
...
#| fig-cap: Cargas estandarizadas del CFA de Redes Sociales
 ...
 cat("RESUMEN DEL MODELO CFA")
indices <- fitmeasures(redes_cfa, c("rmsea", "cfi", "tli"))
cat("RMSEA =", round(indices["rmsea"], 3), "\n")
cat("CFI =", round(indices["cfi"], 3), "\n")
cat("TLI =", round(indices["tli"], 3), "\n\n")
...
tabla_seg_obj <- t_seg_obj %>%
  kable(
    format = "html",
    caption = "Descriptivos del índice Seguridad Objetiva",
...
tabla_seg_sub <- t_seg_sub %>%
  kable(
    format = "html",
    caption = "Descriptivos del índice Seguridad Subjetiva",
    col.names = c("Variable", "Media", "DE", "Rango", "Distribución", "α"),
    digits = 2,
    align = c("l", "c", "c", "c", "c", "c")
  ) %>%
...
tabla_pert <- t_pert %>%
  kable(
    format = "html",
    caption = "Descriptivos del índice Pertenencia al Barrio",
    col.names = c("Variable", "Media", "DE", "Rango", "Distribución", "α"),
    digits = 2,
    align = c("l", "c", "c", "c", "c", "c")
  ) %>%
...
tabla_satis <- t_satis %>%
  kable(
    format = "html",
    caption = "Descriptivos del índice Satisfacción Con el Barrio",
    col.names = c("Variable", "Media", "DE", "Rango", "Distribución", "α"),
    digits = 2,
    align = c("l", "c", "c", "c", "c", "c")
  ) %>%
...
tabla_conf <- t_conf %>%
  kable(
    format = "html",
    caption = "Descriptivos del índice Confianza Interpersonal",
    col.names = c("Variable", "Media", "DE", "Rango", "Distribución", "α"),
    digits = 2,
    align = c("l", "c", "c", "c", "c", "c")
  ) %>%
...
tabla_prosoc <- t_prosoc %>%
  kable(
    format = "html",
    caption = "Descriptivos del índice Comportamiento Prosocial",
    col.names = c("Variable", "Media", "DE", "Rango", "Distribución", "α"),
    digits = 2,
    align = c("l", "c", "c", "c", "c", "c")
  ) %>%
...
tabla_ayuda <- t_ayuda %>%
  kable(
    format = "html",
    caption = "Descriptivos del índice Ayuda Económica",
    col.names = c("Variable", "Media", "DE", "Rango", "Distribución", "α"),
    digits = 2,
    align = c("l", "c", "c", "c", "c", "c")
  ) %>%
