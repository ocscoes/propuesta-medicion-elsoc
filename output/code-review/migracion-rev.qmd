...
# Dimensión: Seguridad subjetiva y Variables de migración
...
seguridad_por_ola_migracion_ordered <- seguridad_por_ola_migracion_ordered %>%
  arrange(r12_04, ola)
...
elsoc_long <- elsoc_long %>%
  mutate(restriccion_migrantes = case_when(
    c37_05 %in% c(1, 2) ~ "En desacuerdo",
    c37_05 == 3 ~ "Ni de acuerdo ni en desacuerdo",
    c37_05 %in% c(4, 5) ~ "De acuerdo",
    TRUE ~ NA_character_
  ))

# order from  "En desacuerdo", "Ni de acuerdo ni en desacuerdo", "De acuerdo"
elsoc_long$restriccion_migrantes <- factor(elsoc_long$restriccion_migrantes, levels = c("De acuerdo", "Ni de acuerdo ni en desacuerdo", "En desacuerdo")) 
